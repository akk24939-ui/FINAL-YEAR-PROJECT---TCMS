...
def enable_teetotaler(
    ...
    return toggle_teetotaler(current_user, enabled=True, db=db)


@router.post("/disable", response_model=ConsumerProfileResponse)
def disable_teetotaler(
    current_user: User = Depends(get_current_consumer),
    db: Session = Depends(get_db),
):
    """Disable teetotaler mode."""
    return toggle_teetotaler(current_user, enabled=False, db=db)
